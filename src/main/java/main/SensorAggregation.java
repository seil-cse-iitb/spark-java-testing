...
//		rows = rows.select(timeField).where(rows.col("sensor_id").equalTo(sensorId)).orderBy(rows.col(timeField).desc()).limit(1);
		rows = rows.select(functions.max(timeField),functions.first("sensor_id")).where(rows.col("sensor_id").equalTo(sensorId));
		if (rows.count() > 0) {
			...
			rows = rows.select(functions.max(timeField)).where("sensor_id='" + sensorId + "'");
			if (rows.count() > 0) {
				Row firstRow = rows.first();
				this.startTS = firstRow.getDouble(firstRow.fieldIndex(timeField));
			} else {
